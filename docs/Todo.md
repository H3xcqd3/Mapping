# Todo

## Current: v0.1 Toolchain

- [x] Configure local paths in `config/map-build.ini`.
- [ ] Complete `TOOLCHAIN_SETUP.md`.
- [x] Perform a minimal RenX to W3D export.
- [x] Create the LevelEdit mod package and test output.
- [x] Run validation with no errors.

## Next: v0.2 Terrain

- [ ] Ask the designer to approve terrain dimensions and topology options.
- [ ] Create the first terrain blockout.
- [ ] Establish collision and world boundaries.
- [ ] Pass client and dedicated-server loading tests.
